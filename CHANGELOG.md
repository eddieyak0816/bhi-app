...
- **"Advanced Care Planning" → "Advance Care Plan"** — text-only rename per Damon's request, 3 spots: `bhasV2.ts` (score breakdown label), `ProfilePage.tsx`, `QuickMetricsPanel.tsx`.
- **Health Check-In stuck on "Loading…" forever** — two layered bugs. First: `HealthAssessmentModal.tsx` used `.single()` to load the most recent check-in, which throws a 406 for any first-time user (zero rows is normal, not an error) with no `.catch()` to handle it. Fixed by switching to a direct REST fetch (returns a plain array — no special zero-rows handling needed at all). Second, deeper layer found after retesting: even after that fix it could still hang intermittently after navigating through several pages first — same `getSession()`-deadlock class of bug as the thumbnail upload. Converted both the load *and* save paths off the Supabase client entirely onto direct `fetch()` + JWT-from-localStorage, matching the now-established safe pattern.
- **Resources loading slowly after entering lab values** — `EvaluationContext.tsx`'s `reevaluate()` was re-downloading the *entire* `logic_rules`, `resources`, and `tags` tables from scratch on every single lab result change, regardless of what actually changed. Added a 5-minute in-memory cache for this shared (non-user-specific) content, and converted the fetch off the Supabase client onto direct REST for the same deadlock-avoidance reason as above.
- **Signup stuck on "Creating account…" forever when using an org invite code** — the invite-code join call to the backend (Render) had no timeout; a Render free-tier cold start (can take 50+ seconds) left it hanging indefinitely with no way for the UI to recover. Added a bounded 55s timeout via the existing `withTimeout()` helper.
- **New org members not actually joining their org at signup** — the invite-code join was only attempted `if (data.session?.access_token)`, but a session is usually *not* available yet at that point since email confirmation is required first — so the join was silently skipped every time, even though the account itself was created fine. Fixed: if no session is available yet, the invite code is now saved (`localStorage`) and automatically applied the moment the user's first real session appears (on the `SIGNED_IN` auth event, e.g. after confirming their email), instead of being dropped.
- **Signup email confirmation link pointed to `localhost:3000`** — Supabase's "Site URL" auth setting was left on its local-development default. Root-caused and reproduced; fixed by Damon directly in the Supabase dashboard (required Owner/Admin-level access this session's account didn't have).
- **Signup emails hitting Supabase's rate limit** — same root cause category as above: still using Supabase's default built-in email sender (testing-only, low send cap). Fixed by Damon connecting a real email provider in Supabase.
