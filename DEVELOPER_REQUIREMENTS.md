...
| 71 | My Team Dashboard card | ✅ Built | User-facing team card on Dashboard. Shows team name, org name, user's rank within team, and team leaderboard (team names + avg BHAS % only — no individual PHI). GET /api/my-team endpoint. MyTeamCard.tsx component. Hidden automatically if user has no team assignment. Built 2026-05-11. |
| 72 | Super Admin lab results viewer | ✅ Built | GET /api/admin/lab-results with filters; AdminLabResultsTab.tsx; value colour-coded vs normal range; expandable inline history. PHI-safe: shows username + public_id only. Built 2026-05-11. |
| 73 | Edit marker ranges via wizard UI | ✅ Built | Admin → Markers ✎ Edit modal shows same 3-tier multi-range UI as New Marker Wizard Step 2. Pre-populates from existing logicRules. Save calls PATCH /api/admin/lab-markers/:id + PUT /api/admin/lab-markers/:id/rules. Built 2026-05-13. |
| 74 | Lab marker applicable_sex field | ✅ Built | applicable_sex TEXT (male/female/both) on lab_markers; sex badge in Admin table/card; sex filter dropdown; LabsPage manual entry filters by user sex. Migration: 20260513_add_applicable_sex_to_lab_markers.sql. Built 2026-05-13. |
| 75 | Video Category Preferences | ✅ Built | Replaces Health Goals. user_category_preferences table; ProfilePage per-category toggles; Dashboard filters recommendations by preferred categories. Migration: 20260513_create_user_category_preferences.sql. Built 2026-05-13. |
| **Phase 11 – HIPAA Hardening** | | ❌ 0/6 complete | |
| 76 | Session timeout (inactivity auto-logout) | ❌ Not built | Track mousemove/keydown; logout after ~15 min inactivity. Add to App.tsx as a hook. |
| 77 | Audit logging | ❌ Not built | audit_log table (user_id, action, target_table, target_id, ip_address, timestamp). Write from Express server on all PHI access/mutation endpoints. |
| 78 | Verify + lock down Storage bucket | ❌ Not built | Confirm PDFs are not stored in public bucket. Verify resource-thumbnails bucket has no health data. Check Supabase dashboard Storage policies. |
| 79 | Groq PHI review (PDF extraction) | ❌ Not built | Raw lab PDFs sent to Groq may contain name/DOB/provider. Confirm Groq BAA status or switch to memory-only processing with no external call for PHI fields. |
| 80 | profiles table PHI audit | ❌ Not built | Review whether name/email columns are still needed alongside username. If not, restrict or remove to reduce PHI surface area. |
...
| **Phase 13 – NHLS Score & Lab Marker Redesign** | | 🔶 1/4 complete | *Confirmed by Damon email 2026-05-20* |
| 83 | Dashboard: NHLS v2.3 panel as sole score | ✅ Built | Hormone Health v1 banner removed 2026-05-24. NHLS v2.3 derived-ratio panel is now the only score on Dashboard. Hormone health prompt + Labs link retained. |
...
| **Phase 14 – Dashboard UX & Lab Tracking Improvements** | | ❌ 0/3 complete | *Requested by Damon 2026-06-25 (Email Exchange 8)* |
| 89 | Core metric quick-entry on Dashboard | ❌ Not built | Inline collapsible panel on Dashboard for entering the 8 NHLS score metrics (Vitamin D, B12, hs-CRP, HOMA-IR inputs, HbA1c, waist+height for WtHR, TG+HDL, ACP checkbox). Writes to `user_lab_results` via existing save flow. Triggers score recalculation + trigger message popup. **Resolved:** Panel starts collapsed. **Still open:** Replace or additive to Labs page entry? |
| 90 | Initial labs + 3–6 month follow-up lab sets | ❌ Not built | New `lab_sets` table: `id, label TEXT, sort_order INT, is_initial BOOL` — Admin-managed timeframe labels (e.g. "Initial", "3 Month Follow-Up", "6 Month Follow-Up"). Add `lab_set_id UUID → lab_sets` to `user_lab_results`. Users assign a lab entry to a set at time of entry; can also reset their initial baseline by reassigning. Side-by-side comparison of all sets for the 8 NHLS metrics shown on both Dashboard and Labs page (Damon to decide later if one location should be removed). NHLS score always uses most recent values. Admin UI: manage timeframe labels (add/edit/delete). Requires DB migration. |
| 91 | Video coverage audit | ❌ Investigation needed | Query `resources` table to count tagged videos per NHLS marker. For each of the 8 scored metrics, report how many resources fire when that marker is non-optimal. Goal: confirm users see ≥5 recommendations for common lab patterns. May result in Damon needing to add more tagged content. This is a data/admin task, not a code task. |
| **Phase 15 – Nav Links Self-Service** | | ✅ 1/1 complete | *Requested by Damon Aug 2026 ("provide instructions for adding additional affiliate links to the top navigation")* |
| 92 | Admin-managed nav dropdown links (multi-menu) | ✅ Built | New `nav_links` table (`db/migrations/20260819_create_nav_links.sql` + `20260819b_add_nav_links_group.sql` adding `group_label`) — independent from `affiliate_products` so deleting a product doesn't remove it from the nav and vice versa. Supports **multiple separate nav dropdown menus**, not just one: each link has a `group_label` (the dropdown's title, e.g. "25% Off Supplements"), and the header renders one dropdown per distinct group that has ≥1 active link — typing a new group name on a link creates a brand-new dropdown automatically, no separate setup step. New Admin tab `AdminNavLinksTab.tsx` (add/edit/delete/reorder within group/activate, grouped list view with a "Hidden — no active links" indicator). `Layout.tsx`'s nav now reads live from this table via direct REST fetch instead of a hardcoded array — Damon can add/remove/reorder links and whole new menus himself with no code deploy needed. |
