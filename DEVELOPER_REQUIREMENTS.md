...
| 92 | Admin-managed nav dropdown links | ✅ Built | New `nav_links` table (`db/migrations/20260819_create_nav_links.sql`) — independent from `affiliate_products` so deleting a product doesn't remove it from the nav and vice versa. New Admin tab `AdminNavLinksTab.tsx` (add/edit/delete/reorder/activate). `Layout.tsx`'s nav dropdown now reads live from this table via direct REST fetch instead of a hardcoded array — Damon can add/remove/reorder links himself with no code deploy needed. |
